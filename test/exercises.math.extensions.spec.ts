...
describe('Extension API', function () {
    xit('1 / 3 + 1 / 4 = (1 * 4 + 1 * 3 ) / ( 3 * 4 ) = 7 / 12', () => {
        ...
        assert.equal(ee.extensions[0].operands[0].toString(), '1,4,1,3');
        assert.equal(ee.extensions[0].operands[1].toString(), '3,4');
        assert.equal(ee.extensions[0].value.toString(), '7,12');
    });

    xit('2 / 3 + 1 / 6 = (2 * 2 + 1 ) /  6  = 5 / 6', () => {
        const expr: Expression = {
            operations: ['add'],
            operands: [[2,3] , [1,6]],
            value: [5,6]
        ...
        const ee: ExtensionExpression = extendAddFraction({expression:expr}).extension
        assert.equal(ee.extensions[0].operands[0].toString(), '2,2,1');
